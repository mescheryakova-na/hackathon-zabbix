...
class LogWriter {
    ...
    public function __construct($log_file = 'error.txt', $params = array()){
        ...
        $this->log_file = $baseDirectory . '/' . $log_file;
        $this->params = array_merge($this->options, $params);

        $baseDirectory = realpath(dirname(__FILE__) . '/../');
        $this->checkDirectory(dirname($this->log_file));
    ...
    protected function checkDirectory($dirname) {
        if (is_dir($dirname)) {
            return true;
        }
        ...
        if ($pos !== false) {
            ...
            if ($this->checkDirectory(substr($dirname, 0, $pos))) {
